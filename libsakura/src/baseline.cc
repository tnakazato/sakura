...
inline void DoSubtractBaseline(
		...
		double *model_data, float clipping_threshold_sigma, unsigned int clipping_max_iteration,
	...
	for (size_t i = 0; i <= clipping_max_iteration; ++i) {
...
inline void SubtractBaselinePolynomial(
		...
		unsigned int clipping_max_iteration, bool get_residual,
		float *out) {
	size_t num_model = order + 1;
