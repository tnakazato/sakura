...
#define ADDSUFFIX(A, B) CONCAT(A, B)
#define STR(A) (# A)
#define STR2(A) STR(A)
...
namespace {
  ...
  inline integer at3(integer B, integer C,
		     integer a, integer b, integer c) {
    return a*(B*C) + at2(C, b, c);
  }
  inline integer at4(integer B ,integer C, integer D,
		     integer a, integer b, integer c, integer d) {
    return a*(B*C*D) + at3(C, D, b, c, d);
  }

  void gridPos(double const xy[/*2*/],
	       integer sampling,
	       integer loc[/*2*/],
	       integer off[/*2*/]) {
  ...
  bool onGrid (integer nx, integer ny, integer loc[/*2*/], integer support) {
    ...
    return (x - support >= 0) && (x + support < nx) && 
      (y - support >= 0) && (y + support < ny);
  }

  namespace ForSpace {
    struct WeightOnly {
      static inline Gridding::value_t
      func(
	   float weight,
	   Gridding::value_t const values/*[nrow][nvischan]*/[/*nvispol*/],
	   integer nvischan, integer nvispol,
	   integer irow, integer ichan, integer ipol) {
    ...
    struct WeightedValue {
      ...
      func(
	   ...
	   integer irow, integer ichan, integer ipol) {
	return weight *
	  conj(values[at3(nvischan, nvispol, irow, ichan, ipol)]);
      }
    };

    template <typename T>
    inline void
    doGrid(Gridding::value_t const values/*[nrow][nvischan]*/[/*nvispol*/],
	...
	if (weight_ > 0.0)  {
	  for (integer ipol=0; ipol < nvispol; ++ipol) {
	    ...
	    if (! flag[at3(nvischan, nvispol, irow, ichan, ipol)]) {
	      Gridding::value_t nvalue =
		T::func(weight_, values, nvischan, nvispol, irow, ichan, ipol);
	      float norm = 0.0;
	      integer ir = 0;
    ...
    struct Adapter {
      ...
      static inline void
      check(integer nvispol, integer npol,
	    integer const polmap[/*nvispol*/],
	    integer nvischan, integer nchan,
	    integer const chanmap[/*nvischan*/]) {
	assert(0 < npol);
	assert(0 < nvispol);
      }

      static inline FuncType
      chooseFunc(bool dowt, integer nvispol, integer npol) {
	return gridFuncs[dowt];
      }

      static inline void
      callFunc(FuncType func,
	       Gridding::value_t const values/*[nrow][nvischan]*/[/*nvispol*/],
	       integer nvispol, integer nvischan,
	       Gridding::flag_t const flag/*[nrow][nvischan]*/[/*nvispol*/],
	       float const weight/*[nrow]*/[/*nvischan*/],
	       Gridding::value_t grid[],
	       float wgrid[],
	       integer nx, integer ny, integer npol, integer nchan,
	       integer support,
	       float const convTable[],
  ...
  namespace ForSpeed {
    ...
    struct WeightedValue {
      ...
      func(
	   ...
	   integer irow, integer ichan, integer ipol) {
	return weight *
	  conj(values[at2(nvischan, irow, ichan)][ipol]);
      }
    };
	...
	for (integer ix=0; ix < Wsupport; ++ix) {
	  ...
	  for (integer ichan=0; ichan < nvischan; ++ichan) {
	    ...
	    for (integer ipol=0; ipol < nvispol; ++ipol) {
	      integer apol = polmap[ipol];
	      float wt_ = wt * (nop &&  (! flag[at2(nvischan, irow, ichan)][ipol]));
	      Gridding::value_t nvalue =
		T::func(weight_, values, nvischan, irow, ichan, ipol);
#if 1
...
#undef FUNCTYPE
  ...
  template <typename T>
  inline void
  internalGridsd(double const xy[/*nrow*/][2],
	 Gridding::value_t const values/*[nrow][nvischan]*/[/*nvispol*/],
	 integer nvispol, integer nvischan, 
	 bool dowt,
	 Gridding::flag_t const flag/*[nrow][nvischan]*/[/*nvispol*/],
	...
	if (onGrid(nx, ny, loc, support)) {
	  integer irad[square(Wsupport)];
	  {
	    ...
	    for (integer iy=0; iy < Wsupport; ++iy) {
	      ...
	      for (integer ix=0; ix < Wsupport; ++ix) {
		rlocx += sampling;
		irad[ir] = static_cast<integer>(sqrt(square(rlocx) +
						     square(rlocy)));
		++ir;
	      }
	    }
...
namespace sakura {
  void ADDSUFFIX(Gridding, ARCH_SUFFIX)::gridsd
		(double const xy[/*nrow*/][2],
		 ...
		 integer const polmap[/*nvispol*/],
		 double sumwt/*[nchan]*/[/*npol*/]) {
    internalGridsd<ForSpace::Adapter>(xy,
		   values,
		   nvispol, nvischan, 
		   dowt,
		   flag,
  ...
  void ADDSUFFIX(Gridding, ARCH_SUFFIX)::gridsdForSpeed
		(double const xy[/*nrow*/][2],
		 ...
		 integer const polmap[/*nvispol*/],
		 double sumwt/*[nchan]*/[/*npol*/]) {
    internalGridsd<ForSpeed::Adapter>(xy,
		   values,
		   nvispol, nvischan, 
		   dowt,
		   flag,
  ...
  void ADDSUFFIX(Gridding, ARCH_SUFFIX)::transform
	(integer ny, integer nx, integer nchan,  integer npol,
	 ...
	 float wgridTo/*[nchan][npol][ny]*/[/*nx*/]) {
