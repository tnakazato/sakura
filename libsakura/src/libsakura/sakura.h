 ...
 * and @a out. @a num_in must be positive.
 ...
 * @details
 * @param[in] num_data 配列 @a data 、 @a mask 、 @a out 、及び、モデルを構成する各基底関数の離散的データ点の要素数。正値でなければならない。
 * @param[in] data 入力データ。要素数は @a num_data でなければならない。
 * @param[in] mask 入力データに対するマスク情報。要素数は @a num_data でなければならない。値がfalseの要素に対応する入力データはフィッティングに用いられない。
 * @param[in] num_model_bases モデルを構成する基底関数の数。正値でなければならない。
 * @param[in] model モデルを構成する全ての基底関数の離散的な値を格納する１次元配列。データに対するループは関数に対するループより内側になる。即ち、 @a m 番目のモデル関数の @a n 番目のデータ点の値は、 @a model [ @a num_data * ( @a m -1) + ( @a n -1)]に格納されなければならない。配列の長さは( @a num_model_bases * @a num_data )でなければならない。
 ...
 * @details
 * @param[in] num_data 配列 @a data 、 @a mask 、 @a out の要素数。正値でなければならない。
 * @param[in] data 入力データ。要素数は @a num_data でなければならない。
 * @param[in] mask 入力データに対するマスク情報。要素数は @a num_data でなければならない。値がfalseの要素に対応する入力データはフィッティングに用いられない。
 * @param[in] order 多項式モデルの次数。ゼロまたは正値でなければならない。
 * @param[in] clipping_threshold_sigma クリッピングの閾値。単位はσ。正値でなければならない。
 * @param[in] num_fitting_max 再帰的フィッティングを行う最大回数。正値でなければならない。値nが与えられた場合、最初のフィッティング＆差し引きを行った後、残差データのσを計算し、残差がその値の± @a clipping_threshold_sigma 倍を越えるものを除外して再度フィッティング＆差し引きを行うという操作を最大(n-1)回繰り返す。デフォルト値は1、即ち、フィッティング＆差し引きは１回のみ行われ、クリッピングは行わない。
 ...
 * @details
 * @param[in] num_each_basis モデルを構成する各基底関数の離散的データ点の要素数。正値でなければならない。
 * @param[in] order 多項式モデルの次数。ゼロまたは正値でなければならない。
