...
// called for the first coefficient
	inline bool Init(const Scalar& value, const ScararOther &isInvalid, int i,
			int j) {
		if (isInvalid) {
			return false;
		}
...
inline void stat(DataType const *data, bool const *isInvalid, size_t elements,
	...
	assert(LIBSAKURA_SYMBOL(IsAligned)(isInvalid));
	Map<Array<bool, Dynamic, 1>, Aligned> isInvalid_(
			const_cast<bool *>(isInvalid), elements);
...
inline float add_horizontally(__m256 packedValues) {
	...
	float total;
	_mm_store_ss(&total,
			_mm_add_ss(_mm256_castps256_ps128(packedValues),
...
inline int32_t add_horizontally(__m256i packedValues) {
	__m128i count2 = _mm256_castsi256_si128(
			_mm256_permute2f128_si256(packedValues, packedValues, 1) );
	__m128i count4w = _mm_hadd_epi32(_mm256_castsi256_si128(packedValues),
			count2);
...
void simd(float const *data, bool const *isInvalid, size_t elements,
		LIBSAKURA_SYMBOL(StatisticsResult) &result) {
...
void ADDSUFFIX(Statistics, ARCH_SUFFIX)::Reduce(float const *data,
		bool const *isInvalid, size_t elements,
		LIBSAKURA_SYMBOL(StatisticsResult) &result) const {
#if 0
	stat(data, isInvalid, elements, result);
#else
#if defined( __AVX__)
	simd(data, isInvalid, elements, result);
#endif
#endif
}
}
