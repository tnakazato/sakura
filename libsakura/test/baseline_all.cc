...
void SetFloatPolynomial(size_t num_coeff, double const *coeff, size_t num_data,
		float *data) {
...
void Create(LIBSAKURA_SYMBOL(Status) status,
	...
	if (mybaseline_type == LSQFitTypeInternal_kPolynomial) {
		...
		create_status = sakura_CreateLSQFitContextSinusoidFloat(order, num_data,
				context);
	}
...
struct FitExecute {
	...
	static void execute(LSQFitTypeInternal const mybaseline_type,
			struct sakura_LSQFitContextFloat const * context, size_t order,
			size_t const * nwave, size_t num_data, float const data[],
			bool mask[],
			//bool const mask[],
			float clip_threshold_sigma, uint16_t num_fitting_max,
			size_t num_coeff, double (coeff[])[4], float best_fit[],
			float residual[],
			bool final_mask[], size_t * boundary, double *coeff_answer_ptr,
		...
		if (mybaseline_type == LSQFitTypeInternal_kPolynomial) {
			...
			for (NPCases item = static_cast<NPCases>(0); item < NP_kNumElems;
					item = static_cast<NPCases>(item + 1)) {
				cout << np_cases_names[item]
						<< ((item < NP_kNumElems - 1) ? ", " : "");
				...
				LIBSAKURA_SYMBOL(LSQFitPolynomialFloat)(context, order,
						num_data, data, mask, clip_threshold_sigma,
						num_fitting_max, num_coeff,
						//todo
						coeff_ptr[0], best_fit_ptr, residual_ptr, final_mask,
						&rms, &baseline_status);
		...
		if (mybaseline_type == LSQFitTypeInternal_kSinusoid) {
			...
			for (NPCases item = static_cast<NPCases>(0); item < NP_kNumElems;
				...
				LIBSAKURA_SYMBOL(Status) fit_status =
				LIBSAKURA_SYMBOL(LSQFitSinusoidFloat)(context, order, nwave,
						num_data, data, mask, clip_threshold_sigma,
						num_fitting_max, num_coeff,
						//todo
						coeff_ptr[0], best_fit_ptr, residual_ptr, final_mask,
		...
		if (mybaseline_type == LSQFitTypeInternal_kCubicSpline) {
			for (NPCases item = static_cast<NPCases>(0); item < NP_kNumElems;
				...
				LIBSAKURA_SYMBOL(Status) fit_status =
				LIBSAKURA_SYMBOL(LSQFitCubicSplineFloat)(context, order,
						//num_pieces,
						num_data, data, mask, 5.0f, 1, coeff_ptr, best_fit_ptr,
						residual_ptr, mask, &rms, boundary, &baseline_status);
...
struct InitializePoly {
	static void execute(LIBSAKURA_SYMBOL(Status) &status,
			LSQFitTypeInternal &mybaseline_type, size_t dummy1[],
			//size_t const dummy2[],
			uint16_t order[], float clip_threshold_sigma[],
			uint16_t num_fitting_max[], size_t num_coeff[],
			size_t num_coeff_answer[], size_t *num_data) {
...
struct InitializeSinusoid {
	static void execute(LIBSAKURA_SYMBOL(Status) &status,
			LSQFitTypeInternal &mybaseline_type, size_t *num_nwave,
			uint16_t *nwave_max, float * clip_threshold_sigma,
			uint16_t * num_fitting_max, size_t * num_coeff,
			size_t * num_coeff_answer, size_t * num_data) {
...
struct InitializeCubicSpline {
	static void execute(LIBSAKURA_SYMBOL(Status) &status,
			LSQFitTypeInternal &mybaseline_type, size_t *num_pieces,
			uint16_t *npieces, float * clip_threshold_sigma,
			uint16_t * num_fitting_max, size_t * num_coeff,
			size_t * num_coeff_answer, size_t * num_data) {
...
void TestRun0(LIBSAKURA_SYMBOL(Status) status,
		LSQFitTypeInternal const mybaseline_type, int16_t const order,
...
void TestRun2(LIBSAKURA_SYMBOL(Status) status,
		...
		size_t const num_data, float *data_ptr,
		bool *mask_ptr, float clip_threshold_sigma, uint16_t num_fitting_max,
		size_t num_coeff, double * coeff_ptr, float * best_fit_ptr,
		float * residual_ptr,
		bool * final_mask, float * rms_ptr, size_t const * boundary, //cubic
	...
	T_fitter::execute();
	if (context != nullptr) {
		T_fitter::execute(mybaseline_type, context, order, nwave, num_data,
				data_ptr, mask_ptr, clip_threshold_sigma, num_fitting_max,
				num_coeff, coeff_ptr, best_fit_ptr, residual_ptr, final_mask,
				//rms_ptr,
				boundary,
				//baseline_status,
				coeff_answer);
...
void TestRun3(LIBSAKURA_SYMBOL(Status) status,
	...
	T_initializer::execute(status, mybaseline_type, &num_nwave_pieces,
			&order_nwavemax_npiece, &clip_threshold_sigma, &num_fitting_max,
			&num_coeff, &num_coeff_answer, &num_data);
	...
	if (mybaseline_type == LSQFitTypeInternal_kPolynomial) {
		SetFloatPolynomial(ELEMENTSOF(coeff_answer), coeff_answer, num_data,
				data);
		for (size_t i = 0; i < num_coeff_answer; ++i) {
		...
		SetFloatSinusoidal(num_nwave_pieces, nwave, coeff_answer, num_data,
				data);
	...
	T_creator::execute(status, mybaseline_type, order_nwavemax_npiece, num_data,
			&context);
	//T_fitter::execute();
	if (context != nullptr) {
		T_fitter::execute(mybaseline_type, context, order_nwavemax_npiece,
				nwave, num_data, data, mask, clip_threshold_sigma,
				num_fitting_max, num_coeff, coeff, best_fit, residual, mask,
				boundary, coeff_answer, num_coeff_answer);
		delete[] nwave;
		T_destroyer::execute(status, context);
	}
}
...
TEST_F(Baseline, CreateLSQFitContextFloatWithCubicSplineUsingNpiece10IR_Numdata12OR) {
	...
	TestRun0<CreateExecute, NullExecute, NullExecute>(
			...
			LSQFitTypeInternal_kCubicSpline, npiece, num_data);
}
...
TEST_F(Baseline, CreateLSQFitContextFloatWithSinusoid_Nwave10IR_Numdata30IR) {
	...
	TestRun0<CreateExecute, NullExecute, DestroyExecute>(
			LIBSAKURA_SYMBOL(Status_kOK), LSQFitTypeInternal_kSinusoid, nwave,
			num_data);
}
...
TEST_F(Baseline, CreateLSQFitContextFloatWithSinusoid_Nwave10IR_Numdata21OR) {
	...
	TestRun0<CreateExecute, NullExecute, NullExecute>(
			LIBSAKURA_SYMBOL(Status_kInvalidArgument),
			LSQFitTypeInternal_kSinusoid, nwave, num_data);
}
...
TEST_F(Baseline, DestroyLSQFitContextFloatWithContextNullPointer) {
	...
	TestRun0<NullExecute, NullExecute, DestroyExecuteNuLL>(
			LIBSAKURA_SYMBOL(Status_kInvalidArgument),
			LSQFitTypeInternal_kCubicSpline, order, num_data);
}
